...
router.get('/count/:city', (req, res) => {
  Property.find({ city: req.params.city })
    .count()
    .then((docs) => {
      res.json(docs);
    })
...
router.get('/count/for/:for', (req, res) => {
  Property.find({ for: req.params.for })
    .count()
    .then((docs) => {
      res.json(docs);
    })
...
router.get('/my/:email', (req, res) => {
  Property.find({ email: req.params.email })
    ...
    .catch((err) =>
      res
        .sendStatus(400)
        .json({ failed: 'Error While Getting your Properties' })
    );
});
...
router.delete('/my/delete/:id', (req, res) => {
  Property.findOneAndDelete(req.params.id)
    .then((docs) => res.json(docs))
    .catch((err) =>
      res.sendStatus(400).json({ failed: 'Error While Deleting' })
    );
});
